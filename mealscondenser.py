...
ignoreList = ['Across the Bridge Foundation', 'ALL SOULS CHURCH SCHOOL', 'Cristo Rey San Jose High School', 'Design Tech High', 'EE S Residential Group Home Inc.', 'Escuela Popular', 'Escuela Xochitl Tonatiul', 'JT Residential Care Facilities', 'Latino College Prep Academy', 'Navigator Schools', 'Sacred Heart Nativity School', 'SAN MATEO CO JUV HALL SCHS', 'SANTA CLARA CO PROBATION', 'South Valley Charter School', 'St Andrew Residential Programs for Youth (STAR HOUSE)', 'ST ELIZABETH SETON SCHOOL', 'ST PATRICK ELEM CATHOLIC', 'Unity Care Group Inc.', 'Voices College-Bound Language', 'REBEKAH CHILDRENS SERVICES']
...
for row in csv_f:
  if (rowNum != 0):
    countyName = row[0]
    schoolDistrictName = removeCommas(row[5])
    if (isEligibleCounty(countyName) and (not(schoolDistrictName in ignoreList))):
      ...
      if (isBreakfast(mealType)):
        schoolDistrictToFreeBreakfastsMap[schoolDistrictName] = schoolDistrictToFreeBreakfastsMap[schoolDistrictName] + readNumber(row[9])
        schoolDistrictToRPBreakfastsMap[schoolDistrictName] = schoolDistrictToRPBreakfastsMap[schoolDistrictName] + readNumber(row[10])
        schoolDistrictToPaidBreakfastsMap[schoolDistrictName] = schoolDistrictToPaidBreakfastsMap[schoolDistrictName] + readNumber(row[11])

      if (isLunch(mealType)):
        schoolDistrictToFreeLunchesMap[schoolDistrictName] = schoolDistrictToFreeLunchesMap[schoolDistrictName] + readNumber(row[9])
        schoolDistrictToRPLunchesMap[schoolDistrictName] = schoolDistrictToRPLunchesMap[schoolDistrictName] + readNumber(row[10])
        schoolDistrictToPaidLunchesMap[schoolDistrictName] = schoolDistrictToPaidLunchesMap[schoolDistrictName] + readNumber(row[11])
